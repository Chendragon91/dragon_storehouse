...
def write_to_hive(df, table_name, partition_date):
    ...
    df.write \
        .mode('append') \
        .insertInto(f"tms_dws.{table_name}")

def process_org_deliver_stats(partition_date: str, table_name: str):
    """处理转运站派送成功统计信息并写入Hive"""
    spark = get_spark_session()
    print(f"[INFO] 开始处理转运站派送成功统计信息，目标分区日期：{partition_date}")
    source_dt = "20250713"  # 源数据分区日期

    # 1. 读取源数据（源表分区为20250713）
    source_df = spark.table("tms_dws.dws_trans_org_deliver_suc_1d") \
...
if __name__ == "__main__":
    target_table = 'dws_trans_org_deliver_suc_nd'
    target_partition = '20250725'  # 目标分区字段值
    process_org_deliver_stats(target_partition, target_table)