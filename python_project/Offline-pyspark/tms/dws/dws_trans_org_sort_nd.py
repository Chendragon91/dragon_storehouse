...
def process_org_sort_stats(partition_date: str, table_name: str):
    ...
    print(f"[INFO] 开始处理机构分拣统计信息，目标分区日期：{partition_date}")
    source_dt = "20250713"  # 源数据分区日期

    # 1. 读取源数据（源表分区为20250713）
    source_df = spark.table("tms_dws.dws_trans_org_sort_1d") \
...
if __name__ == "__main__":
    target_table = 'dws_trans_org_sort_nd'
