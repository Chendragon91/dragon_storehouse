...
def process_order_cargo_detail(partition_date: str, tableName):
    ...
    # 2. 处理ods_order_info表 - 修复estimate_arrive_time转换问题
    info_df = spark.table("tms.ods_order_info") \
        ...
        col("distance"),
        when(
            (col("status") == "60080") | (col("status") == "60999"),
            substring(col("update_time"), 1, 10)
        ).otherwise(lit("9999-12-31")).alias("end_date")
    )
    ...
    result_df = cargo_df \
        ...
        col("distance"),
        date_format(to_timestamp(col("order_time"), "yyyy-MM-dd HH:mm:ss"), "yyyy-MM-dd").alias("start_date"),
        col("end_date"),
        col("end_date").alias("dt")  # 使用end_date作为分区字段
    )
...
if __name__ == "__main__":
    target_table = 'dwd_trade_order_process_inc'
