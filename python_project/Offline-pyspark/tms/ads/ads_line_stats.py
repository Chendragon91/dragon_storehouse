...
def write_to_ads_line_stats(df: DataFrame, target_date: str):
    """写入目标表ads_line_stats，增加类型校验和错误处理"""
    try:
        # 1. 校验目标表结构
        target_table = "tms_ads.ads_line_stats"
...
def execute_line_stats_etl(source_date: str, target_date: str):
    spark = get_spark_session()
    print(f"[INFO] 开始执行线路统计ETL，源数据日期：{source_date}，目标分区日期：{target_date}")

    # 1. 处理多天数据（nd表）
    line_stats_nd = spark.table("tms_dws.dws_trans_shift_trans_finish_nd") \
        .filter(col("dt") == source_date) \
        .groupBy("recent_days", "line_id", "line_name") \
        .agg(
        ...
        .withColumn("dt", lit(target_date))  # 使用目标分区日期
    ...
    final_df = line_stats_nd.select(target_columns)

    # 3. 写入目标表
    write_to_ads_line_stats(final_df, target_date)
...
if __name__ == "__main__":
    ...
    execute_line_stats_etl(source_date, target_date)