...
def write_to_ads_order_cargo_type_stats(df: DataFrame, target_date: str):
    """写入目标表ads_order_cargo_type_stats，增加类型校验和错误处理"""
    try:
        # 1. 校验目标表结构
        target_table = "tms_ads.ads_order_cargo_type_stats"
...
def execute_order_cargo_type_stats_etl(source_date: str, target_date: str):
    spark = get_spark_session()
    print(f"[INFO] 开始执行货物类型订单统计ETL，源数据日期：{source_date}，目标分区日期：{target_date}")

    # 1. 处理1天数据（1d表）
    cargo_type_order_1d = spark.table("tms_dws.dws_trade_org_cargo_type_order_1d") \
    ...
    # 2. 处理多天数据（nd表）
    cargo_type_order_nd = spark.table("tms_dws.dws_trade_org_cargo_type_order_nd") \
        .filter(col("dt") == source_date) \
        .groupBy("recent_days", "cargo_type", "cargo_type_name") \
        .agg(
        sum("order_count").alias("order_count"),
        sum("order_amount").alias("order_amount")
    ) \
    ...
    # 3. 合并所有数据
    final_df = cargo_type_order_1d.unionByName(cargo_type_order_nd)
    ...
    # 5. 写入目标表
    write_to_ads_order_cargo_type_stats(final_df, target_date)
...
if __name__ == "__main__":
    ...
    execute_order_cargo_type_stats_etl(source_date, target_date)